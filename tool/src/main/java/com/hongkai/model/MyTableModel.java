...
public class MyTableModel  extends AbstractTableModel {
    ...
    public MyTableModel(List<Map<String,Object>> rs, String[] DScolumnName, String[] columnName) throws Exception {
        ...
        try {
           for(Map<String,Object> amap:rs) {
                String[] row = new String[column];
                for (int i = 0; i < column; i++) {
                    Object value=amap.get(DScolumnName[i]);
                    if(value==null){
                        row[i] = "";
                    }else{
                        row[i] = amap.get(DScolumnName[i]).toString();
                    }
                }
                resultSet.add(row);
            }
        ...
        setDataVector(newVector(resultSet.size()), newVector(DScolumnName.length-1));
    }
